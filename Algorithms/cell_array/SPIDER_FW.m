function [solution, obj_values] = SPIDER_FW(W1, Xtrain, ytrain, Xtest, ytest, num_iters, batch_size, eta_coef, eta_exp, epoch_length, loss_fn, grad_fn, lmo_fn, gap_fn, evaluate_accuracy_fn, print_freq, IS_ADVERSARIAL, IS_CALCULATING_REGRET, optimal_solution)
    if IS_ADVERSARIAL
    ...
    for t = 1 : num_iters
        ...
        if mod(t - 1, epoch_length) == 0
            ...
            for tmpi = 1 : length(grad_estimate)
                grad_estimate{tmpi} = grad_estimate{tmpi} - stoch_grad_old{tmpi} + stoch_grad{tmpi};
            end
