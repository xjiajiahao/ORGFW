function [solution, obj_values] = MORGFW(W1, Xtrain, ytrain, Xtest, ytest, num_iters, batch_size, sub_batch_size, eta_coef, eta_exp, rho_coef, rho_exp, reg_coef, loss_fn, grad_fn, lmo_fn, gap_fn, print_freq, IS_ADVERSARIAL, IS_CALCULATING_REGRET, optimal_solution)
    if ~IS_ADVERSARIAL
        error('Go home, MORGFW only applies to the adversarial setting.');
    end
    %% initialization
    overhead_time = 0.0;
    ...
    for t = 1 : num_iters
        ...
        for k = 1 : num_oracles
            ...
            if k == 1
                ...
                stoch_grad_old = grad_fn(W_old, Xtmp(:, idx), ytmp(idx));
